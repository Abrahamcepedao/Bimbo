...
const Analysis = () => {
    ...
    const calculateAvgResults = (dim: string, arr: IResults[], stkhs_short: IStkh[]) => {
        ...
        arr.forEach((res) => {
            if(res.results.data.filter((el) => tempStkhs.includes(el.stkh)).length !== 0){
                count++
                sum += res.results[dimensionesSum[dim as 'riqueza'] as 'sum_riqueza'] / res.results[dimensionesTotal[dim as 'riqueza'] as 'sum_riqueza_max']! * 100
            }
        })
        return sum / count
    }
    ...
    const calculateAvgResultsStkh = (stkh: string, arr: IResults[]) => {
        ...
        arr.forEach((res) => {
            if(res.results.data.filter((el) => el.stkh === stkh).length !== 0){
                count++
                sum += res.results.data.filter((el) => el.stkh === stkh)[0]?.sum_total / res.results.data.filter((el) => el.stkh === stkh)[0]?.sum_total_max! * 100
            }
        })
    ...
    const handleSetupHistoryDimensions = (table: ITable, res: IResults[], stkhs_short: IStkh[]) => {
        ...
        Object.keys(dimensiones).forEach((dim: string) => {
            let tempObj = {
                dimension: dimensiones[dim as 'riqueza'],
                resultado: table[dimensionesSum[dim as 'riqueza'] as 'sum_riqueza'] / table[dimensionesTotal[dim as 'riqueza'] as 'sum_riqueza_max']! * 100,
                promedio: res.length !== 0 ? calculateAvgResults(dim, res, stkhs_short) : 0,
                ideal: 100,
            }
            tempRdar1.push(tempObj)
    ...
    const handleSetupHistoryStkh = (table: ITable, res: IResults[], stkhs_short: IStkh[]) => {
        ...
        stkhs_short.forEach((stkh) => {
            ...
            let tempObj = {
                dimension: stkh.name,
                promedio: res.length !== 0 ? calculateAvgResultsStkh(stkh.name, res) : 0,
                resultado: (table.data.filter((el) => el.stkh === stkh.name)[0]?.sum_total / table.data.filter((el) => el.stkh === stkh.name)[0]?.sum_total_max!) * 100,
                ideal: 100,
            }
            tempRdar2.push(tempObj)
        })
    ...
    const calculateAvgTotal = (arr: IResults[]) => {
        ...
        let sum = 0
        arr.forEach((res) => { sum += (res.results.sum_total / res.results.sum_total_max!) * 100 })
        return sum / arr.length
    }

    //calculate bar data
    const calculateBarData = (table: ITable) => setBarData([
            { "dimension": "resultado", "value": table.sum_total/table.sum_total_max!*100, "valueColor": "20, 234, 74" },
    ...
    const captureScreenAndDownloadPDF = async () => {
        ...
        pdf.save('resultados_protocolo.pdf');
    };
                        ...
                        <div className='filters_container opacity-100'>
                            ...
                            <ul className='pl-4'>
                                {Object.keys(dimensiones).map((dim: string, i: number) => (
                                    <li key={i} className='text list-disc text-justify'>En la dimensión <span className='bold'>"{dimensiones[dim as 'riqueza']},"</span> te encuentras <span className={`bold ${getLabelColor(getPercentage(radar1[i].resultado, radar1[i].promedio))}`}>{formatNumber(getPercentage(radar1[i].resultado, radar1[i].promedio))}%</span> {getPercentage(radar1[i].resultado, radar1[i].promedio) < 0 ? 'inferior' : 'superior'} al promedio, (resultado: <span className='bold'>{formatNumber(radar1[i].resultado)}</span>; promedio: <span className='bold'>{formatNumber(radar1[i].promedio)}</span>).</li>
                                ))}
                            </ul>
                        </div>
                    )}
                        ...
                        <div className='filters_container opacity-100'>
                            ...
                            <ul className='pl-4'>
                                {radar2.map((dim: any, i: number) => (
                                    <li key={i} className='text list-disc text-justify'>En el stakeholder <span className='bold'>"{dim.dimension},"</span> te encuentras <span className={`bold ${getLabelColor(getPercentage(dim.resultado, dim.promedio))}`}>{formatNumber(getPercentage(dim.resultado, dim.promedio))}%</span> {getPercentage(dim.resultado, dim.promedio) < 0 ? 'inferior' : 'superior'} al promedio, (resultado: <span className='bold'>{formatNumber(dim.resultado)}</span>; promedio: <span className='bold'>{formatNumber(dim.promedio)}</span>).</li>
                                ))}
                            </ul>
                        </div>
                    )} 
